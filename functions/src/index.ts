...
import { inviteAcceptedNotifId, matchInviteNotifId, matchUpdatedNotifId, matchRemovedNotifId, openMatchJoinNotifId, openMatchLeaveNotifId, openMatchFullNotifId } from './ids';
...
export const joinOpenMatch = onCall(async (request) => {
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    if (match.openInviteStatus !== 'open') {
      throw new HttpsError('failed-precondition', `Match invite is ${match.openInviteStatus}`);
    }
    ...
    if (newAllPlayerIds.length >= maxPlayers) {
      ...
      for (const recipientId of allPlayerIds) {
        ...
        if (creatorProfilePic) notifData.senderProfilePic = creatorProfilePic;
...
export const leaveOpenMatch = onCall(async (request) => {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (!match.isOpenInvite || match.openInviteStatus !== 'open') {
      throw new HttpsError('failed-precondition', 'Match is not open for changes');
    }
