...
export const sendPushOnNotificationWrite = firestore
  ...
  .onWrite(async (change, context) => {
    ...
    if (notification.type === 'match_invite') {
      ...
      body = `${notification.senderName} invited you to a ${matchTypeLabel} match`;
    ...
    } else if (notification.type === 'open_match_join') {
      title = 'Player Joined';
      body = notification.message || `${notification.senderName} joined your open match`;
    } else if (notification.type === 'open_match_leave') {
      title = 'Player Left';
      body = notification.message || `${notification.senderName} left your open match`;
    } else if (notification.type === 'open_match_full') {
      title = 'Match Ready!';
...
/**
 * Callable function to join an open match invite.
 * Atomically adds the caller to the player pool. If the match becomes full,
 * randomizes teams and transitions the match to a normal scheduled match.
 */
export const joinOpenMatch = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'Must be authenticated');
  }
  ...
  const result = await db.runTransaction(async (transaction) => {
    ...
    if (!match.isOpenInvite) {
      throw new HttpsError('failed-precondition', 'Not an open invite match');
    }
    if (match.openInviteStatus === 'cancelled') {
      throw new HttpsError('failed-precondition', 'Match invite is cancelled');
    }
    ...
    if (match.openInviteStatus === 'full') {
      ...
      if (creatorId !== callerUid) {
        ...
        if (!creatorPrefs || creatorPrefs.open_match_join !== false) {
          const notifId = openMatchWaitlistJoinNotifId(matchId, callerUid);
          transaction.set(db.collection('notifications').doc(notifId), buildNotification({
    ...
    if (newAllPlayerIds.length >= maxPlayers) {
      ...
      const teams = assignTeams(allPlayerIds, nameMap);
      ...
      // Not full yet — add to pool
      transaction.update(matchRef, {
        allPlayerIds: FieldValue.arrayUnion(callerUid),
        playerPool: newPlayerPool,
        playerPoolNames: newPlayerPoolNames,
        lastModifiedAt: now,
        lastModifiedBy: callerUid,
        lastModifiedByName: callerName,
      });
  ...
  console.log(`joinOpenMatch: match=${matchId}, user=${callerUid}, joined=${result.joined}, isFull=${result.isFull}`);
  return result;
});

/**
 * Callable function to leave an open match invite.
 * Removes the caller from the player pool and notifies the creator.
 */
export const leaveOpenMatch = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'Must be authenticated');
  }
  ...
  await db.runTransaction(async (transaction) => {
    ...
    if (match.createdBy === callerUid) {
      throw new HttpsError('failed-precondition', 'Creator cannot leave — cancel the match instead');
    }
    ...
    if (!isInPool && !isInMatch && !isOnWaitlist) {
      throw new HttpsError('failed-precondition', 'You are not in this match or waitlist');
    }
    ...
    const notifyCreatorOfLeave = (message: string) => {
      if (!creatorPrefs || creatorPrefs.open_match_join !== false) {
    ...
    if (match.openInviteStatus === 'full' && isInMatch) {
      const newAllPlayerIds = (match.allPlayerIds || []).filter((id: string) => id !== callerUid);
      const waitlist: string[] = [...(match.waitlist || [])];
      const waitlistNames: string[] = [...(match.waitlistNames || [])];
      ...
      if (waitlist.length > 0) {
        ...
        const playerDocs = await Promise.all(
          newAllPlayerIds.map((id: string) => transaction.get(db.collection('players').doc(id)))
        );
        for (const doc of playerDocs) {
          if (doc.exists) nameMap[doc.id] = doc.data()!.name || 'A player';
        }

        const teams = assignTeams(newAllPlayerIds, nameMap);
        ...
        if (promotedId !== creatorId) {
          ...
          transaction.update(db.collection('players').doc(creatorId), {
            connections: FieldValue.arrayUnion(promotedId),
            updatedAt: now,
          });
        }

        // Notify promoted player
        const promotedNotifId = openMatchWaitlistPromotedNotifId(matchId, promotedId);
        transaction.set(db.collection('notifications').doc(promotedNotifId), buildNotification({
        ...
        const remainingPool = newAllPlayerIds.filter((id: string) => id !== creatorId);
        const remainingPoolNames: string[] = [];
        const poolPlayerDocs = await Promise.all(
          remainingPool.map((id: string) => transaction.get(db.collection('players').doc(id)))
        );
    ...
    transaction.update(matchRef, {
      allPlayerIds: FieldValue.arrayRemove(callerUid),
      playerPool: newPlayerPool,
      playerPoolNames: newPlayerPoolNames,
      lastModifiedAt: now,
      lastModifiedBy: callerUid,
...
/**
 * Callable function to cancel an open match invite.
 * Only the creator can cancel. Notifies all joined players.
 */
export const cancelOpenMatch = onCall(async (request) => {
  if (!request.auth) {
    throw new HttpsError('unauthenticated', 'Must be authenticated');
  }
  ...
  await db.runTransaction(async (transaction) => {
    ...
    // Notify all joined players (except creator)
    ...
    for (const recipientId of allPlayerIds) {
      ...
      const notifData: Record<string, any> = {
        ...
        matchLocation: match.location || null,
        matchType: match.matchType,
        message: `${callerName} cancelled the open match`,
        createdAt: now,
      };
      if (callerProfilePic) notifData.senderProfilePic = callerProfilePic;
      transaction.set(db.collection('notifications').doc(notifId), notifData);
    }
  });

  console.log(`cancelOpenMatch: match=${matchId}, user=${callerUid}`);
  return { cancelled: true };
});

/**
 * Scheduled function to expire open match invites that have passed their scheduled date.
 * Runs every hour.
 */
export const expireOpenMatches = onSchedule('every 1 hours', async () => {
    ...
    if (match.scheduledDate && match.scheduledDate < now) {
      try {
        await db.runTransaction(async (transaction) => {
          ...
          if (freshData.openInviteStatus !== 'open') return;
          transaction.update(matchDoc.ref, {
            openInviteStatus: 'cancelled',
            status: 'expired',
...
export const createNotificationsOnMatchCreate = onDocumentCreated(
  ...
  async (event) => {
    ...
    // Skip open invite matches — notifications are handled by joinOpenMatch Cloud Function
    if (match.isOpenInvite) return;
...
export const createNotificationsOnMatchUpdate = onDocumentUpdated(
  ...
  async (event) => {
    ...
    // Skip open invite matches — notifications handled by joinOpenMatch/leaveOpenMatch/cancelOpenMatch
    if (after.isOpenInvite && after.openInviteStatus !== 'full') return;
    ...
    for (const recipientId of existing) {
      ...
      const notifData: Record<string, any> = {
        ...
        message: `${senderName} updated the ${matchTypeLabel} match`,
        createdAt: now,
      };
      if (senderProfilePic) notifData.senderProfilePic = senderProfilePic;
      batch.set(db.collection('notifications').doc(notifId), notifData);
    ...
    for (const recipientId of removed) {
      ...
      const notifData: Record<string, any> = {
        ...
        message: `${senderName} removed you from the ${matchTypeLabel} match`,
        createdAt: now,
      };
      if (senderProfilePic) notifData.senderProfilePic = senderProfilePic;
      batch.set(db.collection('notifications').doc(notifId), notifData);
 ...
 */
export const nudgeNewUsersWithoutMatch = onSchedule('every 1 hours', async () => {
  const now = Date.now();
  ...
  for (const playerDoc of playersSnap.docs) {
    ...
    // Skip placeholders
    if (player.pendingClaim === true) continue;
    // Skip players without push tokens
    if (!player.pushTokens || player.pushTokens.length === 0) continue;
    ...
    const notifId = nudgeNewUserNotifId(playerDoc.id);
 ...
 */
export const nudgeInactiveUsersWeekly = onSchedule({
  ...
  for (const playerDoc of playersSnap.docs) {
    ...
    const notifId = nudgeWeeklyNotifId(playerDoc.id, year, isoWeek);
    batch.set(db.collection('notifications').doc(notifId), {
      id: notifId,
