...
exports.superwallWebhook = void 0;
...
const META_CAPI_TOKEN = (0, params_1.defineSecret)('META_CAPI_TOKEN');
const SUPERWALL_WEBHOOK_SECRET = (0, params_1.defineSecret)('SUPERWALL_WEBHOOK_SECRET');
/**
 ...
 */
const META_TEST_EVENT_CODE = (0, params_1.defineString)('META_TEST_EVENT_CODE', { default: '' });
...
exports.superwallWebhook = (0, https_1.onRequest)({
    ...
    let event;
    try {
        const webhook = new svix_1.Webhook(SUPERWALL_WEBHOOK_SECRET.value());
...
async function forwardToMeta(event) {
    ...
    let name;
    if (playerId && !isAlias) {
        const snap = await (0, firestore_1.getFirestore)().collection('players').doc(playerId).get();
        const player = snap.data();
        if (player) {
            metaContext = player.metaContext;
            email = player.email;
            ...
            phone = (player.phoneNumberE164 ?? player.phoneNumber);
            name = player.name;
        }
    }
    if (!metaContext) {
    ...
    const sent = await (0, capi_1.sendAppEvent)({
        eventName: 'Purchase',
    ...
    }, META_DATASET_ID.value(), META_CAPI_TOKEN.value());
    if (!sent) {
...
//# sourceMappingURL=superwallWebhook.js.map