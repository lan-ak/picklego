...
exports.sendPushOnNotificationWrite = v1_1.firestore
    ...
    let pushTokens = recipientDoc.exists
    ...
    if (pushTokens.length === 0 && /^\d+$/.test(notification.recipientId) && notification.matchId) {
        ...
        if (matchDoc.exists) {
            const allIds = matchDoc.data().allPlayerIds || [];
            for (const id of allIds) {
                if (id === notification.recipientId || id === notification.senderId)
                    continue;
                const altDoc = await db.collection('players').doc(id).get();
                if (altDoc.exists) {
                    const altTokens = (altDoc.data().pushTokens || []).filter((t) => expo_server_sdk_1.default.isExpoPushToken(t));
